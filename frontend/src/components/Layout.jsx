...
import { LayoutDashboard, Users, Grid, CreditCard, LogOut, Menu, X, Moon, Sun } from 'lucide-react';
...
const Layout = ({ setAuth }) => {
  ...
  const navItems = [
    ...
    { name: 'Seats', path: '/seats', icon: Grid },
      ...
      <div className="flex h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
        ...
        <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 shadow-2xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex flex-col items-center justify-center h-28 border-b border-slate-800/60 bg-slate-900/50 backdrop-blur-xl">
            <div className="w-10 h-10 bg-gradient-to-br from-teal-400 to-emerald-600 rounded-xl shadow-lg shadow-teal-500/30 flex items-center justify-center mb-3">
              <span className="text-white font-bold text-xl font-outfit">J</span>
            </div>
            <h1 className="text-xl font-bold text-white tracking-wide font-outfit">Jagannath Library</h1>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <header className="flex items-center justify-between h-20 px-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800 z-10 sticky top-0">
            <button
              className="lg:hidden text-slate-500 dark:text-slate-400 hover:text-teal-600 focus:outline-none transition-colors"
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className="p-2.5 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors focus:ring-2 focus:ring-teal-500/20"
              >
                {darkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
              </button>
              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-teal-500 to-emerald-400 flex items-center justify-center text-white font-bold shadow-md ring-2 ring-white dark:ring-slate-800 cursor-pointer">
                A
              </div>
            </div>
          </header>
